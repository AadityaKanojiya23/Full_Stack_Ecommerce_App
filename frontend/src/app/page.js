      ...
      >
        {items.map((prod) => {
          ...
          return (
            <div
              key={prod._id}
              className={`w-64 shrink-0 bg-card-bg border border-border-color rounded-3xl p-4 flex flex-col justify-between group hover:border-orange amore-card shadow-sm transition-all duration-300 relative overflow-hidden ${isOut ? 'opacity-80' : ''}`}
            >
              {/* Wishlist Button */}
              <button
              ...
              <Link href={`/product/${prod.slug}`} className="space-y-3 cursor-pointer relative block">
                <div className="rounded-2xl overflow-hidden h-40 aspect-video border border-border-color relative">
                  ...
                  {prod.isPremium && !isOut && (
                    <span className="absolute bottom-2 left-2 bg-orange text-white font-medium text-[9px] px-2 py-0.5 rounded-full border border-orange/40 z-10">Premium</span>
                  )}
                </div>
              ...
              <div className="mt-3 pt-3 border-t border-border-color flex items-center justify-between relative z-20">
                ...
                <div className="flex gap-1.5">
                  <button
                    onClick={() => !isOut && onQuickView(prod)}
                    disabled={isOut}
                    className={`p-1.5 border border-border-color rounded-full transition-all ${isOut ? 'text-navy/30 cursor-not-allowed bg-cream/50' : 'text-navy hover:bg-cream'}`}
                    title={isOut ? "Out of Stock" : "Quick Customize View"}
                  >
                  ...
                  <button
                    ...
                    title={isOut ? "Out of Stock" : "Add to Shopping Bag"}
                  >
                    <ShoppingBag className="w-4 h-4" />
                  </button>
                </div>
